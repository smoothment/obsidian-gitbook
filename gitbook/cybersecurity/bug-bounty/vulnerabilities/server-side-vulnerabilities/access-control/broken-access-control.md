...
!\[]\(Pasted image 20240918144012.png) !\[]\(Pasted image 20240918144028.png)
...
!\[]\(Pasted image 20240918163719.png) !\[]\(Pasted image 20240918163807.png) !\[]\(Pasted image 20240918163942.png) There we go, we can now access the panel and delete username Carlos.
...
!\[]\(Pasted image 20240918165853.png) We can see our admin userID right there, lets change it from our account to finish the lab:

!\[]\(Pasted image 20240918165954.png) But careful, if we try to submit this API key, we will get an error because we need `carlos` API not admin, so lets, repeat exercise but with a post from `carlos`: !\[]\(Pasted image 20240918170150.png) !\[]\(Pasted image 20240918170220.png)
...
When we login, we can find that we can change our account id: !\[]\(Pasted image 20240918170659.png)
...
If we login with that password, we are inside admin account: !\[]\(Pasted image 20240918171340.png) !\[]\(Pasted image 20240918171346.png) !\[]\(Pasted image 20240918171355.png)
