...
!\[]\(Pasted image 20240916181317.png) Port 80 http service does not have anything interesting within it, but 8089 does, when we go in we find this: !\[]\(Pasted image 20240916181531.png) If we try XSS on that we get this: !\[]\(Pasted image 20240916181615.png) !\[]\(Pasted image 20240916181626.png) This page is vulnerable to XSS and also to SSTI, we know that by trying following payload: `{{5*5}}`
...
`{{ self._TemplateReference__context.joiner.__init__.__globals__.os.popen('cat /etc/passwd').read() }}` !\[]\(Pasted image 20240916182013.png)
...
!\[]\(Pasted image 20240916183201.png) Now that we've got a stable shell, lets start privilege escalation
...
!\[]\(Pasted image 20240916184431.png) Lets use this to read id\_rsa file from root:

!\[]\(Pasted image 20240916184500.png) We can pass it to a file and download that file to our local machine:
...
!\[]\(Pasted image 20240916185011.png) And just like that, we finished this ctf!
