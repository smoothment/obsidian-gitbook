...
```shell-session
smoothment@htb[/htb]$ git clone https://github.com/Bashfuscator/Bashfuscator
smoothment@htb[/htb]$ cd Bashfuscator
smoothment@htb[/htb]$ pip3 install setuptools==65
smoothment@htb[/htb]$ python3 setup.py install --user
```
...
```shell-session
smoothment@htb[/htb]$ cd ./bashfuscator/bin/
smoothment@htb[/htb]$ ./bashfuscator -h
...
```shell-session
smoothment@htb[/htb]$ ./bashfuscator -c 'cat /etc/passwd'
...
```shell-session
smoothment@htb[/htb]$ ./bashfuscator -c 'cat /etc/passwd' -s 1 -t 1 --no-mangling --layers 1
...
```shell-session
smoothment@htb[/htb]$ bash -c 'eval "$(W0=(w \  t e c p s a \/ d);for Ll in 4 7 2 1 8 3 2 4 8 5 7 6 6 0 9;{ printf %s "${W0[$Ll]}";};)"'
